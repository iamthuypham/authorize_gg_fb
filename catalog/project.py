...
@app.route('/catalogs/<int:id>/', methods = ['GET','PUT','DELETE'])
...
def showAllItems(catalog_id):
    ...
    if 'username' not in login_session:
        return render_template('public_items.html', catalog = catalog, items = items)
    else:
        return render_template('items.html', catalog = catalog, session = login_session, items = items)
...
def createNewItem(catalog_id):
    ...
    if 'username' not in login_session:
        return redirect('/login')
    if request.method == 'POST':
        newItem = Item(name = request.form['newItemName'], description = request.form['newItemDescription'], catalog_id = catalog_id, user_id = login_session['user_id'])
        session.add(newItem)
        session.commit()
        flash("New menu item created!")
        return redirect(url_for('showAllItems', catalog_id = catalog_id))
    else:
...
def editItem(catalog_id, item_id):
    ...
    if editItem != [] and request.method == 'POST':
        ...
        return render_template('items_edit.html', catalog = catalog, item = editItem)
...
def deleteItem(catalog_id, item_id):
    ...
    if deleteItem != [] and request.method == 'POST':
        ...
        return render_template('items_delete.html', catalog = catalog, item = deleteItem)
